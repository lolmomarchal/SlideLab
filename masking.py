...
def remove_small_objects(binary_mask, min_size=None, default=True, avoid_overmask=True, overmask_thresh=95, kernel_size= 1):
    ...
    """
    if default:
        min_size = binary_mask.shape[0] * binary_mask.shape[1] * 0.005
    else:
        min_size = min_size
...
def get_whole_slide_mask(wsi_thumb, results):
    ...
    gc.collect()
def is_background(mask, original_image, gray_range=(100, 150), white_range=(200, 255), threshold=0.3):
    ...
    if background_count/total_elements >= threshold or white_condition >= threshold:
        return True
    return False
def tile_mask(tile):
    _, mask, image_c = threshold(tile)
    return np.array((mask > 1).astype(np.uint8))

def clean_up_tile(tile):
    _, mask, image_c = threshold(tile)
    small_objects = remove_small_objects(mask, 512*512*0.005, default= False)
    binary_mask = np.array((small_objects > 1).astype(np.uint8))
    tile_masked = np.copy(tile)
    tile_masked[binary_mask == 0] = 255
    return tile_masked
