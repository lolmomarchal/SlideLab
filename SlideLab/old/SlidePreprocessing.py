...
@profile
def preprocessing(path, patient_id, args, target_method, device):
    error = []
    summary = []
    vars = []
    # instance statistics
            ...
            for _ in range(max_workers):
                thread = multiprocessing.Process(target=worker_png,
                                                 args=(queue, tile_iterator,
                                                       ...
                                                       results, vars, args.normalize_staining, args.remove_blurry_tiles,
                                                       args.blur_threshold, target_method))
                thread.start()
    ...
    if args.normalize_staining:
        ...
        while i < 5:
            ...
            normalized_img = normalizeStaining(np.array(region))
...
def main():
    ...
    if args.output_format == "h5":
        if not args.normalize_staining and not args.remove_blurry_tiles:
            target_method = "process_tile_h5"
        elif not args.remove_blurry_tiles:
            target_method = "process_tile_norm_h5"
        elif not args.normalize_staining:
            target_method = "process_tile_blur_h5"
        else:
            target_method = "process_blur_norm_tile_h5"
    elif args.output_format == "png":
        if not args.normalize_staining and not args.remove_blurry_tiles:
            target_method = "process_tile_png"
        elif not args.remove_blurry_tiles:
            target_method = "process_tile_norm_png"
        elif not args.normalize_staining:
            target_method = "process_tile_blur_png"
        else:
            target_method = "process_blur_norm_tile_png"
    else:
        target_method = None
    ...
    if args.output_format is not None:
        for i, row in tqdm.tqdm(patients.iterrows(), total=len(patients)):
            ...
            if not os.path.isfile(os.path.join(output_path, row["Patient ID"], row["Patient ID"] + ".csv")):
                results = preprocessing(row["Original Slide Path"], row["Patient ID"], args, target_method, device)
                Reports.Reports([results[0]], [results[1]], output_path)


    # encode after all patients have been preprocessed
    if args.output_format is None and args.encode == False:
        print("No output format was flagged, and so encoding will take place automatically.")

    encoding_times = []
    if args.encode or args.output_format is None:
        for i, row in tqdm.tqdm(patients.iterrows(), total=len(patients)):
            ...
            if not os.path.isfile(os.path.join(encoder_path, str(patient_id) + ".h5")):
                ...
                SlideEncoding.encode_tiles(patient_id, tile_dataset, encoder_path, device, high_qual=args.extract_high_quality,
                                           batch_size=args.batch_size, number_of_augmentations=args.augmentations,
