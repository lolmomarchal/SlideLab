...
def save_tiles_QC(coord, norm_tile, output_dir, patient_id, desired_size, desired_mag):
    try:
        ...
        if not LaplaceFilter(norm_tile):
...
def preprocessing(path, patient_id, args):
    ...
    if device == "cuda":
        ...
        blurry_tiles = len(coordinates) - len(metadata_list) if args.remove_blurry_tiles else None
        ...
        df_tiles.to_csv(tiles_path, index=False)
        # how many removed
        blurry_tiles = len(results) - len(results_) if args.remove_blurry_tiles else None
    return summary_(), error
...
def parse_args():
    ...
    parser.add_argument("--gpu_processes", default=1)
    parser.add_argument("--cpu_processes", default=os.cpu_count())
...
def main():
    ...
    results = []
    for i, row in tqdm.tqdm(patients.iterrows()):
